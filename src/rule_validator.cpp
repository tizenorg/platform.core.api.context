...
// For custom fact
bool ctx::rule_validator::is_valid_fact(std::string subject, ctx::json& fact)
{
	ctx::json tmpl = get_template(subject);
	IF_FAIL_RETURN_TAG(tmpl != EMPTY_JSON_OBJECT, false, _E, "Custsom fact: Failed to get template");
	...
	for (std::list<std::string>::iterator it = keys.begin(); it != keys.end(); it++) {
		...
		ctx::json elem;
		ret = tmpl.get(NULL, CT_CUSTOM_ATTRIBUTES, &elem);
		IF_FAIL_RETURN_TAG(ret, false, _E, "Custom fact: invalid key");

		std::string type;
		elem.get(key.c_str(), CT_CUSTOM_TYPE, &type);
		if (type == CT_CUSTOM_INT) {
			int val;
			ret = fact.get(NULL, key.c_str(), &val);
			IF_FAIL_RETURN_TAG(ret, false, _E, "Custom fact: invalid data type");

			ret = check_value_int(elem, key, val);
			...
			ret = check_value_string(elem, key, val_str);
			IF_FAIL_RETURN_TAG(ret, false, _E, "Custom fact: invalid value");
		} else {
