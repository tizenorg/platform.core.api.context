...
EXTAPI int context_trigger_rule_custom_event_create(const char* event_item, const char* provider, context_trigger_logical_type_e logical_type, context_trigger_rule_entry_h* entry)
{
	...
	std::string subject = provider + std::string("::") + event_item;
...
EXTAPI int context_trigger_rule_custom_condition_create(const char* condition_item, const char* provider, context_trigger_logical_type_e logical_type, context_trigger_rule_entry_h* entry)
{
	...
	std::string subject = provider + std::string("::") + condition_item;
...
EXTAPI int context_trigger_remove_custom_item(const char* name)
{
	...
	// Err: Item not supported
	ctx::json subj;
	int error = ctx::request_handler::is_supported(name, &subj);
	IF_FAIL_RETURN_TAG(error == ERR_NONE, error, _E, "Failed to custom supported request");

	std::string subject;
	subj.get(NULL, CT_CUSTOM_SUBJECT, &subject);

	error = ctx::request_handler::write_with_reply(CONTEXT_TRIGGER_SUBJECT_CUSTOM_REMOVE, &subj, NULL);
	IF_FAIL_RETURN_TAG(error == ERR_NONE, error, _E, "Failed to remove custom item: %#x", error);

	ctx::rule_validator::remove_template(name);
...
EXTAPI int context_trigger_publish_custom_item(const char* name, const char* fact)
{
	...
	// Err: Invalid fact
	bool ret = ctx::rule_validator::is_valid_fact(subject, jfact);
	IF_FAIL_RETURN_TAG(ret, CONTEXT_TRIGGER_ERROR_INVALID_DATA, _E, "Invalid fact");

	ctx::json data;
	data.set(NULL, CT_CUSTOM_SUBJECT, subject);
	data.set(NULL, CT_CUSTOM_FACT, jfact);

	error = ctx::request_handler::write_with_reply(CONTEXT_TRIGGER_SUBJECT_CUSTOM_PUBLISH, &data, NULL);
